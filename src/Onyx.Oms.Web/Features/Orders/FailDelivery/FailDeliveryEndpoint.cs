...
namespace Onyx.Oms.Web.Features.Orders.FailDelivery
{
    public class FailDeliveryEndpoint : IEndpoint
    {
        public void MapEndpoint(IEndpointRouteBuilder app)
        {
            ...
            {
                Result result = await sender.Send(new FailDeliveryCommand(id, request.IsReturning, request.Reason));
    ...
    public record FailDeliveryRequest(bool IsReturning, string? Reason);
}
